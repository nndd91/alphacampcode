def scoreboard(who_ate_what)
  glutton_array =
    who_ate_what.map do |each_hash|
      glutton = Glutton.new(each_hash.fetch('name'))
      glutton.chickenwings = each_hash.has_key?('chickenwings') ? each_hash['chickenwings'] : 0
      glutton.hamburgers = each_hash.has_key?('hamburgers') ? each_hash['hamburgers'] : 0
      glutton.hotdogs = each_hash.has_key?('hotdogs') ? each_hash['hotdogs'] : 0
      glutton.calculate_score
      glutton
    end
